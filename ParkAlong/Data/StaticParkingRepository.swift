...
actor StaticParkingRepository: StaticParkingProviding {
    ...
    func options(in viewport: ParkingViewport, plan: ParkingPlan) async -> [ParkingOption] {
        ...
        let candidates = availableLocations.compactMap { location -> Candidate? in
            guard queryViewport.contains(location.coordinate) else { return nil }
            let distance = ParkingRepository.distance(from: location.coordinate, to: viewport.center)
            guard
                  let rule = ruleResolver.resolve(location: location, plan: plan),
        ...
        return viewport.zoomLevel < 13 ? clustered(options, in: viewport) : options
    }
    ...
    func search(_ query: String, near viewport: ParkingViewport, plan: ParkingPlan, limit: Int = 20) async -> [ParkingOption] {
        ...
        return loadLocationsIfNeeded().compactMap { location -> (Candidate, Double)? in
            let haystack = Self.normalizedSearchText("\(location.name) \(location.municipality) \(location.source.name)")
            let haystackTokens = Set(haystack.split(separator: " ").map(String.init))
            let coverage = Double(tokens.intersection(haystackTokens).count) / Double(tokens.count)
            let exact = haystack.contains(normalized) ? 1.0 : 0
            let textScore = max(exact, coverage * 0.8)
            guard textScore >= 0.45,
                  let rule = ruleResolver.resolve(location: location, plan: plan), rule.isEligible else { return nil }
            let distance = ParkingRepository.distance(from: location.coordinate, to: viewport.center)
            let prediction = location.predictionEvidence.flatMap {
                PredictionEngine.staticEstimate(
                    capacity: location.capacity, evidence: $0, archetype: location.archetype,
                    context: demandContext(for: location, plan: plan), forecastDate: plan.arrival
                )
            }
    ...
    private func availableLocations(viewport: ParkingViewport, plan: ParkingPlan) async -> [StaticParkingLocation] {
        var records = loadLocationsIfNeeded()
        if let remote {
            ...
            if let remoteLocations = try? await remote.locations(for: query) {
                records = RemoteParkingMerger.merge(bundled: records, remote: remoteLocations)
            }
        }
        return records
    }
